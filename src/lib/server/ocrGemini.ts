// Gemini OCR provider. Server-only — the API key must never reach the browser.
// Selected when OCR_PROVIDER=gemini (the default); the dispatcher lives in ocr.ts.
//  - runReceiptOcr: raw field extraction (legacy pre-fill; kept for tools).
//  - runGeminiVerification: the auditor. Given what the branch TYPED, it reads
//    the receipt and reports amount/date/currency plus how well the receipt fits
//    the CHOSEN category. Never decides an outcome.
import process from "node:process";
import type { CurrencyCode } from "#/lib/types";
import type { OcrExtraction, OcrVerification, VerifyCtx } from "./ocr";

// `gemini-flash-latest` tracks the current Flash model so it won't 404 when a
// pinned version is retired. Override via GEMINI_MODEL to pin a specific one.
...
/** Verify a submitted expense against its receipt with Gemini. Reads the receipt
 *  and reports the fields + how well it fits the chosen category. Never throws —
 *  on failure returns a null/zero-confidence result so the caller routes to
 *  `need_check`. */
