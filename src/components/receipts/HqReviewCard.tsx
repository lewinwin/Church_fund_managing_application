// A single flagged transaction with HQ's review actions. Used on the Branch
// detail page and in the cross-branch "Needs Check" queue. OCR only routes here;
// every decision (Cancel / Modify / Correct) is HQ's, made through these buttons.
...
function readOcrCheck(raw: JsonValue | null): OcrCheck | null {
...
function CompareRow({
	...
	return (
		<div className="grid grid-cols-[80px_1fr_1fr_auto] items-center gap-2 py-1 text-sm">
...
export function HqReviewCard({
	...
	return (
		<div className="rounded-xl border border-[var(--color-line)] p-4">
			...
			{check ? (
				<div className="mt-3 rounded-lg bg-[var(--color-forest-50)] px-3 py-2">
					<div className="grid grid-cols-[80px_1fr_1fr_auto] gap-2 pb-1 text-xs font-semibold text-[var(--color-muted)]">
						<span />
						<span>Entered</span>
					...
					<CompareRow
						label="Category"
						entered={catText}
						read={check.ocrCategoryGuess ?? `fit ${Math.round(check.categoryFits * 100)}%`}
						ok={check.categoryOk}
					/>
				</div>
			) : (
				...
				<div className="mt-3 space-y-2">
					...
					<div className="flex flex-wrap gap-2">
						...
						<Button
							variant="accent"
							disabled={busy}
							onClick={() => run(() => requestModify(expense.id, note.trim() || null))}
						>
							Modify
						</Button>
						<Button
							variant="danger"
							disabled={busy}
							onClick={() => run(() => cancelExpense(expense.id, note.trim() || null))}
						>
							<X size={15} /> Cancel
						</Button>
					</div>
