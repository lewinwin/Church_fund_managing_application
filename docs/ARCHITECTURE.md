...
| `server/ocr.ts` | **Local receipt reading** — no API key, no network. `runReceiptVerification(dataUrl)` reads the receipt from whichever source fits: **image via Tesseract** (shared worker, bundled `tessdata/eng.traineddata`), **digital PDF via its text layer** (`pdfText.ts`), or a **scanned/image-only PDF rasterized page-by-page** (`pdfRender.ts`) then OCR'd. The text goes to `receiptParse.ts` → `{ocrAmount, ocrDate, …}`. Category is neutralised (`categoryFits = 1`): no semantic model, so HQ judges category. Server-only; never throws. |
...
| `server/pdfRender.ts` | **PDF page rasterization** — `renderPdfPages(dataUrl)` renders each page to a PNG via `unpdf` + `@napi-rs/canvas` (offline, prebuilt native), capped at 10 pages, so scanned PDFs can be OCR'd. Server-only; never throws. |
...
| `scripts/pdf-render-smoke.ts` | Offline PDF rasterization check (`bun run pdf:render`) — renders `scripts/fixtures/scanned-receipt.pdf` to page images. |
